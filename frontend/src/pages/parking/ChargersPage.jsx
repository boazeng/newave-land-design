...
function ChargersPage() {
  ...
  const fetchData = async () => {
    setLoading(true)
    try {
      const resp = await axios.get('/api/chargers/sites')
      setData(resp.data)
    } catch {
      // No data yet
    } finally {
  ...
  const handleSync = async () => {
    setSyncing(true)
    try {
      await axios.post('/api/chargers/sync', null, { timeout: 300000 })
      await fetchData()
    } catch (err) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-sky-50 via-white to-blue-50">
      ...
      <header className="bg-blue-900 shadow-lg">
        <div className="max-w-6xl mx-auto px-6 py-6 flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white">מטענים</h1>
            <p className="text-base text-blue-200">ניהול מטעני AC - קיבוץ לפי אתר</p>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto px-6 py-8">
        <div className="bg-white rounded-2xl shadow-md border border-sky-100 overflow-hidden">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-sky-100">
            <div>
              <h2 className="text-lg font-bold text-blue-900">אתרי מטענים</h2>
              <p className="text-sm text-blue-800/60">מקור: פריוריטי - מכשירים (משפחה: מטעני AC)</p>
            </div>
            <button
              onClick={handleSync}
              disabled={syncing}
                ...
                {loading ? (
                  ...
                  <tr>
                    <td colSpan="5" className="px-4 py-8 text-center text-blue-800/50">
                      אין אתרים עם קוד אתר. לחץ "סנכרון מפריוריטי" או הוסף DCODE למטענים בפריוריטי.
                    </td>
                  </tr>
                ) : (
                  data.map((item, i) => (
                    <tr key={item.site_code || i} className="border-t border-sky-50 hover:bg-sky-50/50">
